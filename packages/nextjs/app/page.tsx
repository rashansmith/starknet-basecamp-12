...
const Home = () => {
  ...
  const formattedBalance = contractBalance ? (Number(contractBalance.toString()) / 10**18).toFixed(6) : "0.00";
  // const formattedBalance = contractBalance 
  //   ? (Number(contractBalance.toString()) / 10 > 0.000001 
  //       ? (Number(contractBalance.toString())).toFixed(6) 
  //       : (Number(contractBalance.toString())).toLocaleString('en-US', {
  ...
  const {sendAsync: increment} = useScaffoldWriteContract({
    contractName: "Counter",
